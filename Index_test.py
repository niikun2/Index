...
def main():
    ...
    st.write('streamlit開始')
    
    # urlから暗号化されたパラメータを取得
    try:
        params = st.query_params
        encrypted_data = params["defprm"]
        st.write(f'defprm:{encrypted_data}を取得しました')
    except KeyError:
        st.write(f'defprmを取得できませんでした {str(e)}')
    ...
    with st.spinner('データを読み込んでいます...'):
        try:
            ...
            st.write(f'nameid:{nameid}を取得しました')
            st.write(f'gmail_adr:{gmail_adr}を取得しました')
            st.write(f'gmail_psw:{gmail_psw}を取得しました')
            st.write(f'json_data:{json_data}を取得しました')         
            if result == None:
                st.write('nameidを取得できませんでした')
                return
        except Exception as e:
            return f"ERROR: {str(e)}"
    st.title("アンケートフォーム")
    ...
    if SpreadSheetCtrl.connect(json_data):
        ...
        st.write('Google スプレッドシートへの接続に成功しました。')
        ...
        st.write('Google スプレッドシートへの接続に失敗しました。')
